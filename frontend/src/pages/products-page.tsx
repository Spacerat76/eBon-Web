...
function MasterData({ activeTab, apiClient, families, onChanged, onSplit, reviewItems, rules, variants, variantsByFamily }: { activeTab: Exclude<ProductPageTab, "review" | "prices">; apiClient: ApiClient; families: ProductFamilyDTO[]; onChanged: () => Promise<void>; onSplit: (item: ProductReviewItemDTO) => void; reviewItems: ProductReviewItemDTO[]; rules: ProductRuleDTO[]; variants: ProductVariantDTO[]; variantsByFamily: Map<number, ProductVariantDTO[]> }) {
  ...
  return <>
    ...
    {activeTab === "structure" ? <div className="grid gap-4 xl:grid-cols-2"><MergeCard families={families} kind="family" message={message} onPreview={() => void previewFamilyMerge()} onSourceChange={setFamilySource} onTargetChange={setFamilyTarget} preview={preview?.kind === "family" ? preview.value : null} source={familySource} target={familyTarget} onApply={() => void applyMerge()} /><MergeCard families={variants.filter((variant) => variant.isActive).map((variant) => ({ id: variant.id, name: `${variant.productFamilyName}: ${variant.name}` }))} kind="variant" message={message} onPreview={() => void previewVariantMerge()} onSourceChange={setVariantSource} onTargetChange={setVariantTarget} preview={preview?.kind === "variant" ? preview.value : null} source={variantSource} target={variantTarget} onApply={() => void applyMerge()} /><Card className="xl:col-span-2"><CardHeader><CardTitle>Produkte trennen</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-zinc-500">Trennungen werden positionsbezogen zuerst als Vorschau berechnet. Manuell bestätigte Zuordnungen bleiben geschützt; bestehende Regeln werden nicht automatisch kopiert.</p>{splitCandidates.length ? <div className="divide-y divide-zinc-100 rounded-md border border-zinc-200 dark:divide-zinc-900 dark:border-zinc-800">{splitCandidates.map((item) => <div className="flex items-center justify-between gap-3 px-3 py-2 text-sm" key={item.receiptItemId}><div><p className="font-medium">{item.description}</p><p className="text-xs text-zinc-500">{item.currentProductVariantName ?? item.currentProductFamilyName}</p></div><Button onClick={() => onSplit(item)} size="sm" variant="secondary"><GitFork className="h-4 w-4" />Trennen</Button></div>)}</div> : <p className="rounded-md border border-zinc-200 p-3 text-sm text-zinc-500 dark:border-zinc-800">Auf der aktuell geladenen Prüflistenseite gibt es keine zugeordnete Position zum Trennen.</p>}</CardContent></Card></div> : null}
