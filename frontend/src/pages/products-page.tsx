...
type ProductTab = "review" | "master-data";
...
export function ProductsPage({ apiClient, hasApiToken }: { apiClient: ApiClient; hasApiToken: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {selected ? <CorrectionDialog families={families} item={selected} loading={loading} onCancel={() => setSelected(null)} onConfirm={(familyId, variantId) => void action("Produktzuordnung korrigiert.", () => apiClient.correctProductReview(selected.receiptItemId, { productFamilyId: familyId, productVariantId: variantId }))} variantsByFamily={variantsByFamily} /> : null}
...
function CorrectionDialog({ families, item, loading, onCancel, onConfirm, variantsByFamily }: { families: ProductFamilyDTO[]; item: ProductReviewItemDTO; loading: boolean; onCancel: () => void; onConfirm: (familyId: number, variantId: number | null) => void; variantsByFamily: Map<number, ProductVariantDTO[]> }) { const [familyId, setFamilyId] = useState(item.suggestedProductFamilyId ?? item.currentProductFamilyId ?? families[0]?.id ?? 0); const [variantId, setVariantId] = useState(item.suggestedProductVariantId ?? item.currentProductVariantId ?? 0); const variants = variantsByFamily.get(familyId) ?? []; return <Dialog title="Produktzuordnung korrigieren"><p className="text-sm text-zinc-500">{item.description}</p><select aria-label="Produktfamilie" className={selectClass} onChange={(event) => { setFamilyId(Number(event.target.value)); setVariantId(0); }} value={familyId}>{families.filter((family) => family.isActive).map((family) => <option key={family.id} value={family.id}>{family.name}</option>)}</select><select aria-label="Produktvariante" className={selectClass} onChange={(event) => setVariantId(Number(event.target.value))} value={variantId}><option value="0">Keine Variante</option>{variants.filter((variant) => variant.isActive).map((variant) => <option key={variant.id} value={variant.id}>{variant.name}</option>)}</select><DialogActions onCancel={onCancel} onConfirm={() => onConfirm(familyId, variantId || null)} disabled={loading || !familyId} label="Bestätigen" /></Dialog>; }
