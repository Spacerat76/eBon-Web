...
- **F-02.3:** Der Parser verwendet **primär** reguläre Ausdrücke und strukturbasiertes Parsen (regelbasiert). Versionierte Händler-/Filialprofile sind geschlossene deklarative JSON-Definitionen und werden anhand normalisierter Händler-/Filialschlüssel sowie eines versionsgebundenen Layout-Fingerprints ausgewählt; Profilcode oder beliebige Skripte werden nie ausgeführt.
  - Ein passendes `ACTIVE`-Filialprofil hat Vorrang vor einem `ACTIVE`-Händlerprofil (leerer Filialschlüssel), danach folgt Legacy. Fingerprint und Fingerprint-Version müssen übereinstimmen. Ein ausgewähltes ungültiges Profil bleibt mit Fehler/Trace erkennbar und darf nicht durch einen scheinbar erfolgreichen Legacy-Parse verdeckt werden; ausschließlich der validierte KI-Fallback darf es ersetzen.
...
- **F-02.3e:** Der KI-Fallback für Parsing muss ein festes JSON-Schema liefern. Antworten außerhalb des Schemas werden verworfen. Bei verworfenem oder fehlgeschlagenem Fallback bleibt ein vorhandener verwendbarer deterministischer Teilparse nach F-02.5/6a `PARSE_REVIEW`, einschließlich seiner Daten, Profilreferenz und Traces; ursprüngliche `PARSE_ERROR` bleiben Fehler. Die Fallback-Diagnose ergänzt den ursprünglichen Fehlergrund. Jeder Versuch wird in `ai_parsing_log` protokolliert.
- **F-02.3f:** Automatische KI-Parsing-Calls im Sync sind pro Sync-Lauf begrenzt (`ai_parsing_sync_call_limit`, Standard `25`). Wird das Limit erreicht, bleibt ein vorhandener verwendbarer deterministischer Teilparse `PARSE_REVIEW`, ansonsten der ursprüngliche `PARSE_ERROR`. Eine klare Meldung erklärt, dass der KI-Fallback wegen Limit übersprungen wurde.
...
- **F-02.3h:** Wenn ein späterer Regelparser-Reparse vollständig und valide ist, darf er ein früheres KI-Parsergebnis ersetzen und `receipt.parse_source = RULE` setzen. Manuelle Edits dürfen dabei nicht still überschrieben werden, soweit sie im jeweiligen Datenmodell abgebildet sind. Dazu gehören explizite Produktentscheidungen `REJECTED` und `NO_PRODUCT`: Regelannahme überspringt geschützte Bons, Einzel-/Bulk-Reparse verlangt `overwriteManualEdits=true`.
...
- **F-02.7d:** Bulk-Reparse, Reparse nach Regelübernahme und automatische Sync-Läufe verwenden immer den gespeicherten Rohtext und führen keine dialogpflichtige Paperless-Rohtextaktualisierung aus.
  - Jeder übernommene Parse ersetzt Profilreferenz, Profilversion und Zeilen-Traces atomar; Legacy-, KI- und leere Ergebnisse ohne Profil löschen vorherige Profilherkunft und Traces. Ein abgelehnter KI-Fallback erhält die Diagnose des ausgewählten Profils. Reparse nach Regelübernahme überspringt manuell bearbeitete Bons, da dieser Vorgang keine ausdrückliche Überschreibungsfreigabe enthält. Einzel-/Bulk-Reparse behalten ihren bestehenden `overwriteManualEdits`-Vertrag.

**Parser-Normalisierung und Validierung:**

- Bei der Aufnahme eines Profils muss jede Item-Regel im Beispiel treffen. Für ein bereits gespeichertes Profil verlangt die Laufzeitvalidierung nicht, dass jeder Artikeltyp im aktuellen Einkauf vorkommt. Alle Schema-, Pflichtfeld-, Anker-, Bereichs-, Capture-, Kollisions- und Budgetprüfungen bleiben aktiv; ein leeres Regelset bleibt ungültig.
- Profil-Schema v1 enthält ausschließlich `schemaVersion = 1`, `anchors`, `fields`, `itemRules` und `lineRules`; unbekannte Eigenschaften (auch verschachtelt), unbekannte Enum-Werte, doppelte JSON-Schlüssel und Typkoerzierung werden abgelehnt. Records und enthaltene Listen/Maps sind unveränderlich. `ProfileFieldRule` kennt nur die Felder von `ParsedReceipt` ohne `items`; `STORE_NAME`, `RECEIPT_DATE` und `TOTAL_AMOUNT` müssen als Pflichtregeln vorliegen. Item-Captures kennen ausschließlich `DESCRIPTION`, `QUANTITY`, `UNIT`, `UNIT_PRICE`, `TOTAL_PRICE`, `DISCOUNT_AMOUNT`, wobei Beschreibung und Gesamtpreis verpflichtend sind; Capture-Verweise sind numerisch und einbasiert. Item-Typen sind `ITEM`, `DISCOUNT`, `DEPOSIT`, `COUPON`; explizite Nicht-Positionsklassen sind `METADATA`, `PAYMENT`, `TOTAL`, `TAX`, `TSE`, `IGNORED_SAFE`. TSE bleibt im späteren Parse-Trace Metadatum. Anker haben eindeutige IDs; Pflichtanker müssen im Beispiel eindeutig vorkommen. Jede Item-Regel referenziert erforderliche Start-/Endanker und muss dazwischen einen Beispieltreffer liefern; die Ankerzeilen selbst sind ausgeschlossen. Optionale Mehrzeilenregeln erfassen angrenzende Beschreibungszeilen `BEFORE`/`AFTER` mit maximal 2–8 Gesamtzeilen einschließlich Preiszeile. Die spätere Interpretation verbindet sie in Dokumentreihenfolge mit Leerzeichen und darf weder Bereichsgrenzen noch fehlende physische Zeilen überspringen.
- Profil-Regex werden mit RE2/J 1.8 synchron und ohne Backtracking ausgeführt, ausschließlich gegen `originalText` einzelner normalisierter Dokumentzeilen, standardmäßig case-sensitive. Backreferences/Lookarounds werden nicht unterstützt; Treffer der Länge null sind unzulässig. Grenzen: 1024 UTF-16-Zeichen je Regex, 4096 kompilierte NFA-Instruktionen, 32 Capture-Gruppen, 64 Treffer je Zeile, 4096 Zeichen je Zeile, 4096 Zeilen und 262144 Zeichen je Dokument, 128 Regeln insgesamt, 131072 JSON-Zeichen und Verschachtelungstiefe 16. Pro vollständiger Profilauswertung gilt zusätzlich ein gemeinsames Arbeitsbudget von 16777216 Einheiten; vor jedem Suchlauf werden `2 × Programmgröße × (verbleibende Zeilenlänge + 1)` Einheiten reserviert, einschließlich Capture-Nachlauf. Überschreitungen werden strukturiert abgelehnt, nie still gekürzt. Item- und Fortsetzungsregeln werden auf allen Beispielzeilen gegen bekannte bzw. explizit deklarierte Summen-, Steuer-, TSE-, Zahlungs- und Metadatenzeilen geprüft, nicht nur auf ihren ersten Treffer oder innerhalb des Positionsbereichs. Validierungsfehler enthalten ausschließlich Fehlercodes und Schema-Pfade, keine Belegtexte, Regexes oder internen Exception-Ursachen. Die Schemavalidierung ersetzt weder die spätere fachliche Zahlen-/Datums-/Summenprüfung noch aktiviert sie ein Profil.
...
- Generisch verworfene Betragszeilen bleiben für akzeptierte dynamische Legacy-Positionsregeln erreichbar, sofern sie keine tatsächlichen Zahlungs-, Steuer- oder Metadatenzeilen sind; ein Produktwort wie `Protein Bar` ist allein kein Zahlungsnachweis.
  - Die kontextuellen Loyalitätspräfixe `Ihr Guthaben`, `Ihr Punktestand`, `Ihr Vorteil` und `Gesammelte Punkte` bleiben Metadaten statt Positionen. Eine bloße Wortnennung innerhalb einer Warenbezeichnung (`Kleid Punkte`, `Gläser Punkte`) oder ein Teilwort (`Vorteilspack`, `Punkteraster`) sperrt die akzeptierte dynamische Regel nicht.
...
- **F-19.14a:** Der interaktive Codex-Produktaudit darf eine offensichtliche, nicht manuell oder bestätigt geschützte Zuordnung ab `0,980` Konfidenz transaktional korrigieren und dabei optional eine fehlende Produktfamilie oder Variante anlegen. Die Korrektur wird als `AI` + `AUTO_ASSIGNED` mit Modellkennung `codex-interactive-audit` protokolliert, nutzt keinen OpenRouter-Aufruf und wird nur angewendet, wenn die im Request übermittelte bisherige Zuordnung noch exakt dem gesperrt gelesenen Datenbankstand entspricht. Manuelle, bestätigte, abgelehnte und `NO_PRODUCT`-Entscheidungen werden über diesen Pfad nie überschrieben.
...
| POST | `/api/products/review/{receiptItemId}/audit-correct` | Eindeutige interaktive Codex-Audit-Korrektur mit `AI`-/`AUTO_ASSIGNED`-Provenienz und Zustandsprüfung anwenden |
